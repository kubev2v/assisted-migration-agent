...
var _ = ginkgo.Describe("VM Migration Exclusion e2e tests", ginkgo.Ordered, func() {
	...
	ginkgo.Context("Migration Exclusion CRUD Operations", func() {
		...
		ginkgo.It("should successfully exclude a VM", func() {
			...
			for _, vm := range result.Vms {
				if vm.Id == testVMID {
					found = true
					gomega.Expect(vm.MigrationExcluded).ToNot(gomega.BeNil(), "MigrationExcluded should not be nil")
	...
	ginkgo.Context("Filtering VMs by migration exclusion status", func() {
		...
		ginkgo.BeforeEach(func() {
			// Get all VMs across all pages to ensure we process every VM
			...
			for {
				...
				ginkgo.GinkgoWriter.Printf("Fetched page %d: %d VMs (total so far: %d)\n", page, len(result.Vms), len(allVMs))

				// Stop if we got fewer VMs than requested (last page)
				if len(result.Vms) < pageSize {
					break
				}
				page++
			}

			gomega.Expect(len(allVMs)).To(gomega.BeNumerically(">", 5), "need at least 6 VMs for testing")
			...
			// Keep the rest as included - explicitly set all to false
			for i := 3; i < len(allVMs); i++ {
				vmID := allVMs[i].Id
			...
			ginkgo.GinkgoWriter.Printf("Setup: %d excluded, %d included VMs (total: %d)\n",
				len(excludedVMIDs), len(includedVMIDs), len(allVMs))
		})
		...
		ginkgo.It("should return only included VMs when filtering by migrationExcluded=false", func() {
			// Act - fetch all pages since there might be more than 100 included VMs
			pageSize := 100
			allFilteredVMs := []v1.VirtualMachine{}
			page := 1
			...
			for {
				...
				allFilteredVMs = append(allFilteredVMs, result.Vms...)
			...
			// Assert
			gomega.Expect(allFilteredVMs).To(gomega.HaveLen(len(includedVMIDs)), "should return exactly %d included VMs", len(includedVMIDs))

			for _, vm := range allFilteredVMs {
				gomega.Expect(vm.MigrationExcluded).ToNot(gomega.BeNil(), "MigrationExcluded should not be nil")
		...
		ginkgo.It("should return all VMs when no exclusion filter is specified", func() {
			...
			gomega.Expect(allVMs).To(gomega.HaveLen(totalExpected), "should return all VMs")
		})
	})
	...
	ginkgo.Context("Filter DSL with migration_excluded", func() {
		...
		ginkgo.It("should support complex filter expressions with migration_excluded", func() {
			...
			filteredResult, err := agentSvc.ListVMs(&service.VMListParams{
				ByExpression: &expression,
			})
