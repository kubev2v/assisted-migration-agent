...
var _ = Describe("InspectorService", func() {
	...
	BeforeEach(func() {
		...
		srv = mustNewInspectorService(st, 10, "").WithCredentialsService(credsSvc)
	})
	...
	Describe("Cancel", func() {
		...
		Context("when inspector is running", func() {
			BeforeEach(func() {
				...
				srv = mustNewInspectorService(st, 10, "").WithCredentialsService(credsSvc).WithInspectionBuilder(builder.builder())
	...
	Describe("Start", func() {
		...
		It("should return VCenterError for invalid credentials", func() {
			...
			srv = mustNewInspectorService(st, 10, "").WithCredentialsService(badCredsSvc)
	...
	Describe("Inspection limit", func() {
		It("should return InspectionLimitReachedError when Start receives more VM IDs than the limit", func() {
			...
			srv = mustNewInspectorService(st, 2, "").
				WithCredentialsService(credsSvc).WithInspectionBuilder(builder.builder())
