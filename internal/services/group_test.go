...
var _ = Describe("GroupService", func() {
	...
	BeforeEach(func() {
		...
		srv = services.NewGroupService(st, &mockInventoryBuilder{})
	})
	...
	Context("ListVirtualMachines", func() {
		...
		It("should return not found for non-existent group", func() {
			// Act
			vms, total, err := srv.ListVirtualMachines(ctx, uuid.New(), services.GroupGetParams{})
	...
	Context("Atomic Transactions", func() {
		...
		BeforeEach(func() {
			...
			realService = services.NewGroupService(st, duckdb_parser.New(st.Querier(), nil))
		})
	...
	Describe("Outbox Event Generation", Ordered, func() {
		...
		Context("Create", func() {
			...
			It("should not have outbox events for created empty groups", func() {
				...
				events, err = st.Outbox().Get(ctx)
				Expect(err).NotTo(HaveOccurred())
				Expect(events).To(HaveLen(0), "Empty groups should emit outbox events")
			})
