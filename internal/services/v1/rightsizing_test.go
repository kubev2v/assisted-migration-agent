...
var _ = Describe("RightsizingService", func() {
	...
	BeforeEach(func() {
		...
		svc = v1.NewRightsizingService(st)
	})
	...
	Describe("TriggerCollection", func() {
		It("should create a report shell in DuckDB and return it immediately", func() {
			svc.WithWorkBuilder(func(reportID string, cfg rsig.Config, discoverVMs bool, st *store.Store, start, end time.Time) *v1.RightsizingCollectionHandle {
				return &v1.RightsizingCollectionHandle{
