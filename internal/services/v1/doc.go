// Package v1 implements the business logic layer for the assisted-migration-agent.
//
...
// # CredentialsService
//
// CredentialsService manages per-source encrypted credentials (vCenter URL,
// username, password). API access passwords are managed separately by the
// accesspassword package.
//
...
//
// Credential fields (username, password) are encrypted with XChaCha20-Poly1305
// using a persistent encryption key. Each encrypted field is self-contained:
// base64(salt || nonce || ciphertext). The URL is stored in plaintext.
//
// Operations:
//   - Save: encrypts credentials with the provided encryption key and stores them
//   - Get: retrieves and decrypts credentials using the provided encryption key
//   - List: returns all credential IDs
//   - Delete: removes credentials by ID (idempotent)
//
// Usage:
//
//	credsSrv := services.NewCredentialsService(store).WithKeyManager(keyManager)
//	err := credsSrv.Save(ctx, keyManager.Key(), "vc-1", models.Credentials{
//	    URL: "https://vcenter.local/sdk", Username: "admin", Password: "secret",
//	})
//	creds, err := credsSrv.Get(ctx, keyManager.Key(), "vc-1")
//
// # Thread Safety
//
// CollectorService:
