...
var _ = Describe("VMService", func() {
	...
	Context("List", func() {
		...
		It("should return all VMs with total count", func() {
			// Act
			vms, total, err := srv.List(ctx, v1.VMListParams{})
		...
		It("should apply pagination", func() {
			// Arrange
			params := v1.VMListParams{Limit: 3, Offset: 0}
		...
		It("should combine sort and pagination", func() {
			// Arrange
			params := v1.VMListParams{
				Sort:  []v1.SortField{{Field: "memory", Desc: true}},
				Limit: 1,
			}
		...
		It("should return empty list for offset beyond total", func() {
			// Arrange
			params := v1.VMListParams{Limit: 10, Offset: 100}
