...
func NewConsoleService(cfg config.Agent, client *console.Client, mgr *ServiceManager, mainStore *store.Store) (*Console, error) {
...
func newConsoleService(cfg config.Agent, client *console.Client, mgr *ServiceManager, store *store.Store, defaultStatus models.ConsoleStatus) *Console {
...
func (c *Console) run(closeCh chan any) {
	...
	for {
		...
		for range ticks {
			// check for closeCh after tick to be sure we're not waiting for the pipeline to stop when closing
			select {
			case <-closeCh:
				return
			default:
		...
		_, pipeErr := pipeline.Result()
		if pipeErr != nil {
			c.state.SetCurrent(models.ConsoleStatusDisconnected)
			c.state.SetError(pipeErr)
			if errors.IsConsoleClientError(pipeErr) {
				zap.S().Named("console_service").Errorw("failed to send request to console. console service stopped", "error", pipeErr.Error())
				c.state.SetFatalStopped()
				return
			}
			interval = min(interval*2, maxBackoffInterval)
			zap.S().Named("console_service").Errorw("failed to dispatch to console", "error", pipeErr, "next_heartbeat", interval.String())
		} else {
...
func (c *Console) createPipeline(s *scheduler.Scheduler[any]) (*work.Pipeline[string, any], error) {
	...
	if eventSrv == nil {
		return work.NewPipeline(s, work.NewSliceWorkBuilder(units, func(_ context.Context, _ any) error { return nil })), nil
	}
