...
type CollectorWorkBuilder interface {
	Build() work.WorkBuilder[models.CollectorStatus, models.CollectorResult]
}

type CollectorService struct {
	ID       string
	mu       sync.Mutex
	pipeline *work.Pipeline[models.CollectorStatus, models.CollectorResult]
...
func (c *CollectorService) Start(ctx context.Context) error {
	...
	p := work.NewPipeline(sched, c.builder.Build())
	ticks, err := p.Start()
	if err != nil {
		sched.Close()
		return err
