...
type InspectorService struct {
	...
	pool            *work.Pool[models.InspectionStatus, models.InspectionResult]
}

func NewInspectorService(st *store.Store, inspectionLimit int, dataDir string, credsSvc *CredentialsService, cfg *config.Agent) *InspectorService {
...
// Pool is created on-demand per request batch to avoid re-entrant Pool execution limits.
func (i *InspectorService) Start(ctx context.Context, vmIds []string) (err error) {
	...
	wb := make(map[string]work.WorkBuilder[models.InspectionStatus, models.InspectionResult])
	for _, vmID := range vmIds {
		wb[vmID] = buildFn(vmID)
	}
	pool := work.NewPool(wb).WithWorkers(defaultStandardWorkers, defaultStandardWorkers).
