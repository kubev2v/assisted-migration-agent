...
func NewInspectorService(st *store.Store, inspectionLimit int, dataDir string, credsSvc *CredentialsService) *InspectorService {
...
func (i *InspectorService) Start(ctx context.Context, vmIDs []string) (err error) {
	...
	wb := make(map[string]work.WorkBuilder[models.InspectionStatus, models.InspectionResult], len(vmIDs))
	...
	pool := work.NewPool(wb).WithWorkers(defaultInspectionWorkers, defaultInspectionWorkers).
