...
func (f *ForecasterService) IsBusy() bool {
	return f.GetStatus().State == models.ForecasterStateRunning
}
...
func (f *ForecasterService) latestCollectionStore() (*store.Store2, error) {
	...
	var collections []*store.Database
	for _, db := range dbs {
		if db.ID == store.MainDatabaseID {
			continue
		}
		collections = append(collections, db)
	}

	if len(collections) == 0 {
		return nil, srvErrors.NewResourceNotFoundError("collection", "latest")
	}

	slices.SortFunc(collections, func(a, b *store.Database) int {
		return b.CreatedAt.Compare(a.CreatedAt)
	})

	return collections[0].Store()
}
