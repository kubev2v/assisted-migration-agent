...
func NewInspectorServiceV2V(st *store.Store, inspectionLimit int, dataDir string, credsSvc *CredentialsService, cfg *config.Agent) *InspectorServiceV2V {
...
func (i *InspectorServiceV2V) Start(ctx context.Context, vmIds []string) (err error) {
	...
	pool := work.NewPool(wb).WithWorkers(defaultV2VWorkers, defaultV2VWorkers).
		WithSerialPipelines().
