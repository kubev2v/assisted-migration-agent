...
type testCollectorWorkBuilder struct {
	buildFn func() work.WorkBuilder[models.CollectorStatus, models.CollectorResult]
}

func (b *testCollectorWorkBuilder) Build() work.WorkBuilder[models.CollectorStatus, models.CollectorResult] {
	return b.buildFn()
}

func mockCollectorBuilder(st *store.Store, connectErr, collectErr, processErr error) services.CollectorWorkBuilder {
...
func blockingCollectorBuilder(gate chan struct{}) services.CollectorWorkBuilder {
	return &testCollectorWorkBuilder{
		buildFn: func() work.WorkBuilder[models.CollectorStatus, models.CollectorResult] {
			return &mockCollectorWorkBuilder{
				units: []work.WorkUnit[models.CollectorStatus, models.CollectorResult]{
...
var _ = Describe("CollectorService", func() {
	...
	BeforeEach(func() {
		...
		Expect(err).NotTo(HaveOccurred())
		credsSvc = services.NewCredentialsService(mainSt)
		...
		srv = services.NewCollectorService(mockCollectorBuilder(st, nil, nil, nil))
	})
	...
	Context("Start", func() {
		...
		It("should set error state when connection fails", func() {
			srv = services.NewCollectorService(
	...
	Context("NewCollectorService with existing inventory", func() {
		It("should start in collected state when inventory exists", func() {
			...
			collectorSrv := services.NewCollectorService(nil)
