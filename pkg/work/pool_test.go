...
func newTestBuilder(finalizeFn func(ctx context.Context, result int) error, units ...work.WorkUnit[string, int]) work.WorkBuilder[string, int] {
...
var _ = Describe("Pool", func() {
	Context("Start", func() {
		It("should run all entries to completion with correct results", func() {
			builders := map[string]work.WorkBuilder[string, int]{
			...
			pool := work.NewPool[string, int](builders)
	...
	Context("SerialPipelines", func() {
		It("runs one pipeline fully before starting the next (no unit interleaving)", func() {
			...
			mkBuilder := func(name string) work.WorkBuilder[string, int] {
			...
			pool := work.NewPool[string, int](builders).WithWorkers(1, 1).WithSerialPipelines()
			Expect(pool.Start()).To(Succeed())
