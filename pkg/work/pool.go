...
type Pool[S any, R any] struct {
	...
	builders        map[string]WorkBuilder[S, R]
...
func NewPool[S any, R any](builders map[string]WorkBuilder[S, R]) *Pool[S, R] {
	return &Pool[S, R]{
		builders:        builders,
...
func (p *Pool[S, R]) WithWorkers(normal, reserved int) *Pool[S, R] {
	p.workers = normal
	p.reservedWorkers = reserved
	return p
}

func (p *Pool[S, R]) WithFinalizer(fn func(ctx context.Context) error) *Pool[S, R] {
	p.finalizeFn = fn
	return p
}
...
func (p *Pool[S, R]) WithSerialPipelines() *Pool[S, R] {
	p.serial = true
	return p
}
...
// hold p.mu.
func (p *Pool[S, R]) launchPipeline(key string) error {
	pipeline := NewPipeline(p.sched, p.builders[key])
...
func (p *Pool[S, R]) Stop() error {
	p.mu.Lock()
	pipes := make([]*Pipeline[S, R], 0, len(p.pipelines))
...
func (p *Pool[S, R]) Cancel(key string) (S, error) {
	p.mu.Lock()
...
func (p *Pool[S, R]) State(key string) (S, error) {
	p.mu.Lock()
	pl, ok := p.pipelines[key]
	p.mu.Unlock()
...
func (p *Pool[S, R]) Result(key string) (R, error) {
	p.mu.Lock()
	pl, ok := p.pipelines[key]
	p.mu.Unlock()
