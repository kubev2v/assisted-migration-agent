...
// FilterVMsByClusterID filters VMs by cluster ID using the VM-to-cluster mapping.
func FilterVMsByClusterID(vms []vspheremodel.VM, clusterID string, vmToClusterID map[string]string) []vspheremodel.VM {
	...
	filtered := make([]vspheremodel.VM, 0, count)
	for _, vm := range vms {
		if vmClusterID, exists := vmToClusterID[vm.Name]; exists && vmClusterID == clusterID {
...
// FilterHostsByClusterID filters hosts by cluster ID.
func FilterHostsByClusterID(hosts []apiplanner.Host, clusterID string, hostToClusterID map[string]string) []apiplanner.Host {
	// Count first to allocate exact capacity
	count := 0
	...
	// Allocate with exact capacity
	filtered := make([]apiplanner.Host, 0, count)
	for _, host := range hosts {
		if host.Id != nil {
			if hostClusterID, exists := hostToClusterID[*host.Id]; exists && hostClusterID == clusterID {
...
// FilterInfraDataByClusterID filters infrastructure data for a specific cluster.
func FilterInfraDataByClusterID(
	infraData models.InfrastructureData,
	clusterID string,
	hostToClusterID map[string]string,
	clusterVMs []vspheremodel.VM,
	...
	return models.InfrastructureData{
...
// CalculateHostPowerStatesForCluster calculates host power states for a cluster.
func CalculateHostPowerStatesForCluster(clusterHosts []apiplanner.Host, hostIDToPowerState map[string]string) map[string]int {
...
// FilterDatastoresByVMs filters datastores to only include those used by the given VMs.
func FilterDatastoresByVMs(
	datastores []apiplanner.Datastore,
	vms []vspheremodel.VM,
	datastoreMapping map[string]string,
	datastoreIndexToName map[int]string,
) []apiplanner.Datastore {
	...
	filtered := make([]apiplanner.Datastore, 0, len(usedDatastoreNames))
...
// FilterNetworksByVMs filters networks to only include those used by the given VMs.
func FilterNetworksByVMs(networks []apiplanner.Network, vms []vspheremodel.VM, networkMapping map[string]string) []apiplanner.Network {
	...
	filtered := make([]apiplanner.Network, 0, len(usedNetworkNames))
