...
func NewCollector(opaPoliciesDir string) *Collector {
	return &Collector{opaPoliciesDir: opaPoliciesDir}
}

func (c *Collector) Collect(ctx context.Context, creds models.VCenterCredentials) (*apiplanner.Inventory, error) {
	...
	inv := c.createInventory(about.InstanceUuid, vms, infraData)
...
func (c *Collector) createInventory(vCenterID string, vms *[]vspheremodel.VM, infraData models.InfrastructureData) *apiplanner.Inventory {
	return &apiplanner.Inventory{
		...
		Vms: apiplanner.VMs{
			...
			CpuCores:  apiplanner.VMResourceBreakdown{Histogram: apiplanner.Histogram{Data: []int{}}},
			RamGB:     apiplanner.VMResourceBreakdown{Histogram: apiplanner.Histogram{Data: []int{}}},
			DiskCount: apiplanner.VMResourceBreakdown{Histogram: apiplanner.Histogram{Data: []int{}}},
			DiskGB:    apiplanner.VMResourceBreakdown{Histogram: apiplanner.Histogram{Data: []int{}}},
			NicCount:  &apiplanner.VMResourceBreakdown{Histogram: apiplanner.Histogram{Data: []int{}}},
		},
...
// FIX: for god sake, please change this method's name..
func (c *Collector) fillInventoryObjectWithMoreData(vms *[]vspheremodel.VM, inv *apiplanner.Inventory) {
	cpuSet := []int{}
	memorySet := []int{}
	diskGBSet := []int{}
	diskCountSet := []int{}
...
func getProvider(creds models.VCenterCredentials) *api.Provider {
...
func getSecret(creds models.VCenterCredentials) *core.Secret {
	return &core.Secret{
		ObjectMeta: meta.ObjectMeta{
			Name:      "vsphere-secret",
			Namespace: "default",
...
func getNetworks(collector *vsphere.Collector, vmsPerNetwork map[string]int) []apiplanner.Network {
	...
	for _, n := range *networks {
		...
		r = append(r, apiplanner.Network{
			...
			VlanId:   &vlanId,
			Dvswitch: &dvNet.Name,
			VmsCount: intPtr(vmsPerNetwork[n.ID]),
		})
	}
	...
	for _, host := range hosts {
		for _, disk := range host.HostScsiDisks {
			...
			for _, topology := range host.HostScsiTopology {
				if !slices.Contains(topology.ScsiDiskKeys, disk.Key) {
					continue
				}
