...
func (l *lexer) Scan() (int, Token, string) {
	...
	tok := illegal //nolint:ineffassign
	pos := l.pos
	val := ""
	...
	if isIdentifierStart(ch) {
		start := l.tokenStart()
		for isIdentifierStart(l.ch) || isDot(l.ch) {
			if isDot(l.ch) {
				l.next()
				if !isIdentifierStart(l.ch) {
					return pos, illegal, "identifier cannot end with a dot or have consecutive dots"
				}
			}
			l.next()
		}
	...
	if isDigit(ch) {
		...
		for isDigit(l.ch) || isDot(l.ch) {
			if isDot(l.ch) {
				if hasDot {
					return pos, illegal, "number with multiple decimal points"
				}
				hasDot = true
			}
			l.next()
