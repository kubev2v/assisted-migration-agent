...
var _ = Describe("SQL Generation", func() {
	...
	Context("defaultMapFn field mappings", func() {
		...
		fields := []fieldCase{
			...
			{"folder", `v_folder`},
			{"host", `v_host`},
			{"smbios_uuid", `v_smbios_uuid`},
			{"vm_uuid", `v_vm_uuid`},
			{"firmware", `v_firmware`},
			{"powerstate", `v_powerstate`},
			{"status", `v_powerstate`},
			{"connection_state", `v_connection_state`},
			{"ft_state", `v_ft_state`},
			{"cpus", `v_cpus`},
			{"memory", `v_memory`},
			{"os_config", `v_os_config`},
			{"os_tools", `v_os_tools`},
			{"dns_name", `v_dns_name`},
			{"ip_address", `v_ip_address`},
			{"storage_used", `v_in_use_mib`},
			{"template", `v_template`},
			{"cbt", `v_cbt`},
			{"enable_uuid", `v_enable_uuid`},
			{"datacenter", `v_datacenter`},
			{"cluster", `v_cluster`},
			{"hw_version", `v_hw_version`},
			{"total_disk_capacity", `total_disk`},
			{"provisioned", `v_provisioned_mib`},
			{"resource_pool", `v_resource_pool`},
			{"issues_count", `issues_count`},
			{"migratable", `migratable`},
			{"disk.key", `dk_disk_key`},
			{"disk.path", `dk_disk_path`},
			{"disk.capacity", `dk_capacity_mib`},
			{"disk.sharing", `dk_sharing_mode`},
			{"disk.raw", `dk_raw`},
			{"disk.shared_bus", `dk_shared_bus`},
			{"disk.mode", `dk_disk_mode`},
			{"disk.thin", `dk_thin`},
			{"disk.controller", `dk_controller`},
			{"disk.label", `dk_label`},
			{"concern.label", `c_label`},
			{"concern.category", `c_category`},
			{"concern.assessment", `c_assessment`},
			{"inspection.status", `inspection_status`},
			{"inspection.error", `inspection_error`},
			{"inspection_concern.label", `ic_label`},
			{"inspection_concern.category", `ic_category`},
			{"inspection_concern.msg", `ic_msg`},
			{"cpu.hot_add", `cpu_hot_add`},
			{"cpu.hot_remove", `cpu_hot_remove`},
			{"cpu.sockets", `cpu_sockets`},
			{"cpu.cores_per_socket", `cpu_cores_ps`},
			{"mem.hot_add", `mem_hot_add`},
			{"mem.ballooned", `mem_ballooned`},
			{"net.network", `net_network`},
			{"net.mac", `net_mac_address`},
			{"net.nic_label", `net_nic_label`},
			{"net.adapter", `net_adapter`},
			{"net.switch", `net_switch`},
			{"net.connected", `net_connected`},
			{"net.starts_connected", `net_starts_connected`},
			{"net.type", `net_type`},
			{"net.ipv4", `net_ipv4_address`},
			{"net.ipv6", `net_ipv6_address`},
			{"net.cluster", `net_cluster`},
			{"datastore.name", `ds_name`},
			{"datastore.hosts", `ds_hosts`},
			{"datastore.address", `ds_address`},
			{"datastore.object_id", `ds_object_id`},
			{"datastore.free", `ds_free_mib`},
			{"datastore.mha", `ds_mha`},
			{"datastore.capacity", `ds_capacity_mib`},
